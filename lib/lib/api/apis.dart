...
import 'package:mygptrainer/models/chart.dart';
...
class APIs {
  ...
  static Future<void> createUser() async {
    ...
    // Create Gemini collection
    final gemini = Gemini(urlImage: '');
    await firestore
        .collection('users')
        .doc(user.uid)
        .collection('gemini')
...
// chats (collection) --> conversation_id (doc) --> messages (collection) --> message (doc)
  ...
  static Future<void> updateGeminiPicture(Gemini gem, File file) async {
    final ext = file.path.split('.').last;
    log('Extension: $ext'); // Debug log statement

    // Use current timestamp for unique file names
    final ref = storage
        .ref()
        .child('gemini/${DateTime.now().millisecondsSinceEpoch}.$ext');

    await ref
        .putFile(file, SettableMetadata(contentType: 'image/$ext'))
        .then((p0) {
      log('Data Transferred: ${p0.bytesTransferred / 1000} kb'); // Debug log statement
    });

    final imageUrl = await ref.getDownloadURL();

    // Generate a new document ID for each image upload
    final newDoc =
        firestore.collection('users').doc(user.uid).collection('gemini').doc();

    // Reference the specific user's gemini document
    await newDoc.set({'url_image': imageUrl});
  }
}
